<?php
// backend/v1/api/pregunta_frecuente/index.php
...
try {
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            if ($id) {
                $stmt = $pdo->prepare("SELECT * FROM pregunta_frecuente WHERE id = :id");
                $stmt->execute(['id'=>$id]);
                $row = $stmt->fetch();
                if (!$row) {
                ...
                $stmt = $pdo->query("SELECT * FROM pregunta_frecuente");
            ...
            case 'POST':
                ...
                foreach (['pregunta','respuesta'] as $f) {
                    if (empty($input[$f])) {
                        http_response_code(400);
                ...
                $stmt_max_id = $pdo->query("SELECT MAX(id) FROM pregunta_frecuente");
                $max_id = $stmt_max_id->fetchColumn();
                $next_id = $max_id ? (int)$max_id + 1 : 1; // Si no hay registros, empezar en 1
    
                $pdo->beginTransaction();
                $stmt = $pdo->prepare(
                    "INSERT INTO pregunta_frecuente (id, pregunta, respuesta, activo)
                     VALUES (:id, :pregunta, :respuesta, 1)"
                );
                $stmt->execute([
                    'id' => $next_id,
                ...
                case 'PUT':
                    ...
                    $fields = array_intersect_key($input, array_flip(['pregunta','respuesta','activo']));
                    if (empty($fields)) {
                        http_response_code(400);
                    ...
                    $fields['id'] = $id;
                    $pdo->beginTransaction();
                    $stmt = $pdo->prepare("UPDATE pregunta_frecuente SET $sets WHERE id=:id");
                    $stmt->execute($fields);
                    $pdo->commit();
        ...
        case 'PATCH':
            ...
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("UPDATE pregunta_frecuente SET activo=:activo WHERE id=:id");
        ...
        case 'DELETE':
            ...
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("DELETE FROM pregunta_frecuente WHERE id=:id");
            $stmt->execute(['id'=>$id]);
            $pdo->commit();
            if ($stmt->rowCount() === 0) {
                http_response_code(404);
