<?php
// backend/v1/api/equipo/index.php
...
try {
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            if ($id) {
                $stmt = $pdo->prepare("SELECT * FROM equipo WHERE id = :id");
                $stmt->execute(['id'=>$id]);
                $row = $stmt->fetch();
                if (!$row) {
                ...
                $stmt = $pdo->query("SELECT * FROM equipo");
            ...
            case 'POST':
                ...
                case 'PUT':
                    ...
                    $fields['id'] = $id;
                    $pdo->beginTransaction();
                    $stmt = $pdo->prepare("UPDATE equipo SET $sets WHERE id=:id");
                    $stmt->execute($fields);
                    $pdo->commit();
        ...
        case 'PATCH':
            ...
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("UPDATE equipo SET activo=:activo WHERE id=:id");
