<?php
// backend/v1/api/info_contacto/index.php
...
try {
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            if ($id) {
                $stmt = $pdo->prepare("SELECT * FROM info_contacto WHERE id = :id");
                $stmt->execute(['id'=>$id]);
                $row = $stmt->fetch();
                if (!$row) {
                ...
                $stmt = $pdo->query("SELECT * FROM info_contacto");
            ...
            case 'POST':
                ...
                foreach (['nombre','texto','texto_adicional'] as $f) {
                    if (empty($input[$f])) {
                        http_response_code(400);
                ...
                $stmt_max_id = $pdo->query("SELECT MAX(id) FROM info_contacto");
                $max_id = $stmt_max_id->fetchColumn();
                $next_id = $max_id ? (int)$max_id + 1 : 1; // Si no hay registros, empezar en 1
    
                $pdo->beginTransaction();
                $stmt = $pdo->prepare(
                    "INSERT INTO info_contacto (id, nombre, texto, texto_adicional, activo)
                     VALUES (:id, :nombre, :texto, :texto_adicional, 1)"
                );
                $stmt->execute([
                    'id' => $next_id,
                    'nombre' => $input['nombre'],
                    'texto' => $input['texto'],
                    'texto_adicional' => $input['texto_adicional']
                ]);
                $pdo->commit();
                ...
                case 'PUT':
                    ...
                    $fields = array_intersect_key($input, array_flip(['nombre','icono', 'texto','texto_adicional','activo']));
                    if (empty($fields)) {
                        http_response_code(400);
                    ...
                    $fields['id'] = $id;
                    $pdo->beginTransaction();
                    $stmt = $pdo->prepare("UPDATE info_contacto SET $sets WHERE id=:id");
                    $stmt->execute($fields);
                    $pdo->commit();
        ...
        case 'PATCH':
            ...
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("UPDATE info_contacto SET activo=:activo WHERE id=:id");
        ...
        case 'DELETE':
            ...
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("DELETE FROM info_contacto WHERE id=:id");
            $stmt->execute(['id'=>$id]);
            $pdo->commit();
            if ($stmt->rowCount() === 0) {
                http_response_code(404);
