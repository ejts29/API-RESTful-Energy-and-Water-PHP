<?php
// backend/v1/api/mantenimiento_info/index.php
...
try {
    switch ($_SERVER['REQUEST_METHOD']) {
        case 'GET':
            if ($id) {
                $stmt = $pdo->prepare("SELECT * FROM mantenimiento_info WHERE id = :id");
                $stmt->execute(['id'=>$id]);
                $row = $stmt->fetch();
                if (!$row) {
                ...
                $stmt = $pdo->query("SELECT * FROM mantenimiento_info");
            ...
            case 'POST':
                ...
                $stmt_max_id = $pdo->query("SELECT MAX(id) FROM mantenimiento_info");
                $max_id = $stmt_max_id->fetchColumn();
                $next_id = $max_id ? (int)$max_id + 1 : 1; // Si no hay registros, empezar en 1
    
                $pdo->beginTransaction();
                $stmt = $pdo->prepare(
                    "INSERT INTO mantenimiento_info (id, nombre, texto, activo)
                     VALUES (:id, :nombre, :texto, 1)"
                );
                $stmt->execute([
                    'id' => $next_id,
                    'nombre' => $input['nombre'],
                ...
                case 'PUT':
                    ...
                    $fields = array_intersect_key($input, array_flip(['nombre','texto','activo']));
                    if (empty($fields)) {
                        http_response_code(400);
                    ...
                    $fields['id'] = $id;
                    $pdo->beginTransaction();
                    $stmt = $pdo->prepare("UPDATE mantenimiento_info SET $sets WHERE id=:id");
                    $stmt->execute($fields);
                    $pdo->commit();
        ...
        case 'PATCH':
            ...
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("UPDATE mantenimiento_info SET activo=:activo WHERE id=:id");
        ...
        case 'DELETE':
            ...
            $pdo->beginTransaction();
            $stmt = $pdo->prepare("DELETE FROM mantenimiento_info WHERE id=:id");
            $stmt->execute(['id'=>$id]);
            $pdo->commit();
            if ($stmt->rowCount() === 0) {
                http_response_code(404);
